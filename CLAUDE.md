...
- Post-processes the rendered `<img>` tags via `scripts/lib/post-images.js`: intrinsic `width`/`height` read off the copied file (CLS), `decoding="async"`, and `loading="lazy"` on every image but the first.
- Writes committable `src/content/blog/<slug>.json` (shape defined in `src/types/blog.ts`). An optional `updated:` frontmatter key records a revision date, used for `dateModified` and the sitemap's `<lastmod>`.
...
- `composables/useSeo.ts` produces the per-page canonical + reciprocal hreflang set (`it`, `en`, `x-default`), the language-aware `og:url`/`og:locale` (`seoOpenGraph`), the Open Graph/Twitter block (`seoSocial`) and the `article:*` block for posts (`seoArticle`). Every view must call `seoSocial` — without it `og:title`/`og:description` fall through to the static English block in `index.html`. `App.vue` owns `htmlAttrs.lang` and the `rel="alternate"` feed link.
- `composables/useStructuredData.ts` emits one schema.org `@graph` per page (`Person` + `WebSite` always, plus `BlogPosting` / `Blog` / `WebPage` / `ProfilePage` and a `BreadcrumbList`), serialized by `jsonLdScript()` into a `<script type="application/ld+json">`. The `BreadcrumbList` and the visible `BreadcrumbTrail.vue` are fed from the **same `trail` array** in each view — Google requires structured data to describe what the page actually shows, so keep them single-sourced.
- `scripts/generate-feed.js` writes `dist/rss.xml` + `dist/en/rss.xml` in `postbuild` (English feed lists only posts with an `en` block). It duplicates the URL layout like the sitemap script does.
- `scripts/generate-sitemap.js` runs in the `postbuild` step and writes `dist/sitemap.xml` + `dist/robots.txt`. `PAGES` in that script must be kept in sync with `src/router/index.ts`, and it **duplicates the `/en` prefix logic** from `src/i18n/routing.ts` (plain Node, can't import TS) — change both together. Each page is emitted once per language with `xhtml:link` alternates; blog URLs are added dynamically from the synced JSON, with `<lastmod>` from the post's `updated` (falling back to `date`) and `<image:image>` entries for the pictures the post embeds. Site URL is overridable via `SITE_URL` env (defaults to the production domain) so preview builds don't leak the wrong canonical host.
- `vercel.json` gives `sitemap.xml`/`robots.txt`/`rss.xml` correct content-type + caching and rewrites everything else to the SPA entry.
